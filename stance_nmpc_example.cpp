...
void initialize()
{
  ...
  QN.setZero();
  Q = Q_read.asDiagonal();
  R = Qu_read * Eigen::Matrix<double, 12, 12>::Identity(); // 力权重
  QN = static_cast<double>(N) * QN_read.asDiagonal();      // 设定状态权重

  // 初始化其他变量
  flow_dynamic_.SetM(15.0);
  flow_dynamic_.SetT(T_step_read);
  flow_dynamic_.SetL(L_read.asDiagonal());
  flow_dynamic_.SetFoot(Eigen::Vector3d(0.0, -0.1, 0.0), Eigen::Vector3d(0.0, 0.1, 0.0), Eigen::Vector3d(1, 1, 1).asDiagonal(), Eigen::Vector3d(1, 1, 1).asDiagonal());
